...
m<-1000
n<-30
pi_o<-.25 #This value is not suppose to be .1
...
#power_wald<-length(Z_wald[Z_wald>=qnorm(a)])/m #This is not when you rejet a left-tailed test; you reject when the test statistic is small (<-1.96)
#power_wald

#### I see my mistake the line should read
power_wald<- 1- length(Z_wald[Z_wald<=qnorm(a/2)])/m
# the sign was the wrong direction and I forgot to divide a by 2
power_wald
# Score test
...
######## should read
power_score<- 1- length(Z_score[ Z_score<=qnorm(a/2)])/m
# power calculation (only keeping test statiscs greater than alpha
#power_score<-length(Z_score[ Z_score>=qnorm(a)])/m #Same here, this is not when you reject left-tailed test. 
power_score
